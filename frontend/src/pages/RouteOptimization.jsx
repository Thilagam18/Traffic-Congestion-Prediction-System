...
const WMO = {
  ...
  45: { desc: "Foggy", emoji: "🌫️", impact: 1 },
  ...
  81: { desc: "Moderate showers", emoji: "🌧️", impact: 1 },
  82: { desc: "Heavy showers", emoji: "⛈️", impact: 2 },
  95: { desc: "Thunderstorm", emoji: "⛈️", impact: 2 },
  96: { desc: "Thunderstorm + hail", emoji: "⛈️", impact: 2 },
  99: { desc: "Heavy thunderstorm", emoji: "⛈️", impact: 2 },
};
...
function predictTraffic(srcCode, dstCode, distanceKm) {
  ...
  if (maxImpact === 2) score += 25;
  else if (maxImpact === 1) score += 12;
  if (distanceKm > 50) score += 6;
  if (distanceKm > 150) score += 6;
  score = Math.min(score, 100);
  const period = isRushHour
    ? isMorningRush ? "Morning rush hour" : "Evening rush hour"
    : isWeekend ? "Weekend, light traffic" : "Off-peak hours";
...
async function fetchRoute(sLat, sLon, dLat, dLon) {
  const res = await fetch(
    `https://router.project-osrm.org/route/v1/driving/${sLon},${sLat};${dLon},${dLat}?steps=true&geometries=geojson&overview=full`
  );
  const data = await res.json();
...
// ── Map component ─────────────────────────────────────────────────────────────
function LeafletMap({ result, mapStyle }) {
  ...
  useEffect(() => {
    ...
    // Move tile layer below route layers
  ...
  useEffect(() => {
    ...
    const routeColor = traffic.level === "High" ? "#ef4444" : traffic.level === "Moderate" ? "#f59e0b" : "#1a73e8";

    // White border (Google Maps style)
    ...
    // Origin pin (green)
    const srcM = L.marker([result.src.lat, result.src.lon], { icon: makePin("#34a853") })
      .addTo(map)
    ...
    // Destination pin (red)
    const dstM = L.marker([result.dst.lat, result.dst.lon], { icon: makePin("#ea4335") })
      .addTo(map)
  ...
  return <div ref={mapRef} style={{ height: 460, width: "100%", borderRadius: "0 0 12px 12px" }} />;
}
    ...
    <>
      ...
      <div style={{ padding: "24px 32px", backgroundColor: "#f8fafc", minHeight: "100vh" }}>
        ...
        <p style={{ margin: "0 0 24px", color: "#64748b", fontSize: 14 }}>
          Real-time routing with live weather, traffic prediction, and Google Maps-style navigation
        </p>
          ...
          <>
            ...
            <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden", marginBottom: 20 }}>
              <div style={{ padding: "12px 20px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontWeight: 700, color: "#1e3a5f", fontSize: 14 }}>🗺️ Route Map</span>
                {/* Map style toggle — Google Maps style */}
                <div style={{ display: "flex", background: "#f1f3f4", borderRadius: 8, padding: 3, gap: 2 }}>
                  {[
                    { key: "road", icon: "🗺️", label: "Road" },
              ...
              <LeafletMap result={result} mapStyle={mapStyle} />
            </div>
            ...
            <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden" }}>
              ...
              <div style={{ maxHeight: 380, overflowY: "auto" }}>
                {result.steps.map((step, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "12px 20px", borderBottom: i < result.steps.length - 1 ? "1px solid #f3f4f6" : "none", background: i === 0 || i === result.steps.length - 1 ? "#f8fafc" : "white" }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: i === 0 ? "#e8f5e9" : i === result.steps.length - 1 ? "#e3f2fd" : "#f3f4f6", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>
        ...
        {!result && !loading && !error && (
          <div style={{ textAlign: "center", padding: "60px 20px", color: "#9ca3af" }}>
            ...
            <div style={{ fontSize: 14 }}>Live routing · Google Maps-style tiles · Weather at both ends · Traffic prediction · Turn-by-turn directions</div>
          </div>
        )}
      </div>
    </>
